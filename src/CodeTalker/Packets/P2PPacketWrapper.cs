...
internal class P2PPacketWrapper {
    ...
    // For sending packets
    internal P2PPacketWrapper(string sig, byte[] rawData, P2PPacketType type, uint targetNetId = 0) {
        PacketSignature = sig;
        PacketType = type;
        ...
        // Make header
        // Header format: CODE_TALKER_P2P_SIGNATURE PacketSignatureLength(1 Byte) PacketSignature(PacketSigLen bytes long) packetType(1 Byte) targetNetId(4 bytes)
    ...
    internal P2PPacketWrapper(byte[] rawPacketData) {
        ...
        PacketType = (P2PPacketType)rawPacketData[offset++];
